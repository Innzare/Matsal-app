...
export const PaginationItem: FC<PaginationItemProps> = ({
  ...
  useAnimatedReaction(
    ...
    ({ slideProgress }) => {
      ...
      if (slideProgress === 1 && currentSlideIndex === totalSlides - 1) {
        isDragging.set(true);
        translateY.set(withTiming(-topCarouselOffset, { duration: 200, easing: Easing.inOut(Easing.quad) }));
      }
  ...
  return (
    <Animated.View className="h-[2px] rounded-full bg-stone-300 overflow-hidden" style={rBarWidthStyle}>
      <Animated.View className="absolute top-0 bottom-0 left-0" style={[{ backgroundColor: '#EA004B' }, rBarProgressStyle]} />
    </Animated.View>
  );
};
